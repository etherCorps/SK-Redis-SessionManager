...
import type {CookieSerializeOptions, nodeRedisSessionOptions, Serializer} from '$lib/shared.js';
import {
...
export class RedisSessionStore {
    ...
    async updateSession(cookies: Cookies, sessionData = {}): Promise<{ data: any; error: boolean; message: string }> {
        ...
        await this.redisClient.set( keyWithPrefix, serializedSessionData )
        if (this.useTTL && this.ttlSeconds) {
            await this.redisClient.expire( keyWithPrefix, this.ttlSeconds )
        }
